...
class Road:
    def __init__(self, width, total_height, lane_count=3, window_width=800, window_height=600):
        ...
        bottom_left = {'x': self.x, 'y': self.y + self.total_height}
        bottom_right = {'x': self.x + self.width, 'y': self.y + self.total_height}

        self.borders = [[top_left, bottom_left], [top_right, bottom_right]]

    def get_borders(self):
        return self.borders
    
    def get_linepoints(self):
        left_line = ((self.x, self.y), (self.x, self.y + self.total_height))
        right_line = ((self.x + self.width, self.y), (self.x + self.width, self.y + self.total_height))
    ...
    def draw(self, screen, camera_y):
        ...
        # pygame.draw.line(screen, border_color, (self.x,self.y+self.total_height-camera_y), (self.x,self.y-camera_y),5)
        # pygame.draw.line(screen, border_color, (self.x + self.width,self.y-camera_y), (self.x + self.width,self.y + self.total_height-camera_y),5)
